...
class LetorDataset(AbstractDataset):
    ...
    def _load_data(self):
        ...
        with open(self._path, "r") as fin:
            ...
            for line in fin:
                ...
                relevence = float(
                    cols[0]
                )  # Sometimes the relevance label can be a float.
    ...
    def _normalise(self, query):
        ...
        if norm.shape[0] != 1:
            ...
            for docid in self._query_docid_get_features[query].keys():
                self._query_docid_get_features[query][
                    docid
                ] = self._query_get_all_features[query][docid]
    ...
    #     for docid, feature in self._query_docid_get_features[query].items():
    #         if np.array_equal(feature.astype(np.float32), action.astype(np.float32)):
    #             return docid
