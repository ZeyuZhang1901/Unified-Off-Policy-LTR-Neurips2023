...
class Raw_data:
    def __init__(self, data_path=None, file_prefix=None, click_model_dir=None, rank_cut=None, logtype="svm"):
        """
        ...
        if os.path.isfile(data_path + file_prefix + '/' +
                          file_prefix + '.feature') and logtype == "svm":  # files in ULTRA data format
    ...
    def load_data_in_libsvm_format(
        ...
        for line in feature_fin:
            ...
            self.features.append([0.0 for _ in range(self.feature_size)])
            for x in arr[2:self.feature_size+2]:
                arr2 = x.split(':')
                feautre_idx = int(arr2[0]) - 1
                if feautre_idx < self.feature_size:
                    self.features[-1][int(feautre_idx)] = float(arr2[1])
...
def read_data(data_path, file_prefix, click_model_dir, rank_cut=None, logtype="svm"):
    data = Raw_data(data_path, file_prefix, click_model_dir, rank_cut, logtype)
    return data
