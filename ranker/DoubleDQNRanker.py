...
class DoubleDQNRanker(AbstractRanker):
    ...
    def get_next_scores_and_indexs(
        ...
        """Compute ranking scores and indexs with the given inputs.
        ...
        Returns:
            two lists of `[batch_size * 1]` tensors, each with rank_size items.
        """
    ...
    #                 .to(torch.float32)
    #             )
    #             docid = dataset.get_docid_by_query_and_feature(
    #                 batch.qid[i], actions[i].cpu()
    #             )
    #             batch.chosen[i][docid] = False
    #             candidates = candidates[batch.chosen[i]]
    #             next_action = self.selectAction(
    #                 state=nextstates[i].view(1, -1), candidates=candidates
    #             )
    #             next_action = torch.tensor(next_action).reshape(1, -1).to(self.device)
    #             Q_nexts_nexta[i, 0] = self.target_q(
    #                 state=nextstates[i].view(1, -1), action=next_action
    #             )
