...
echo "running state embedding: ATTENTION, output node: `state_dimension`"
rank_json_file=$output_fold/ranker_json/ATTENTION.json
mkdir -p $output_fold/results_cql_alpha/ATTENTION
python ./runs/run_SAC_CQL.py --output_fold $output_fold/results_cql_alpha/ATTENTION \
    --ranker_json_file $rank_json_file \
    --running_json_file $running_json_file \
    --start_epoch $start_epoch 
python ./runs/run_SAC_CQL.py --output_fold $output_fold/results_cql_alpha/ATTENTION \
