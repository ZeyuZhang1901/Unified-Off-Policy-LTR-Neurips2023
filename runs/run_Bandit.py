...
from ranker.BanditRanker import BanditRanker
...
def train(
    ...
    if start_checkpoint > 0:
        ...
        ckpt = torch.load(checkpoint_path + f"Bandit(step_{start_checkpoint}).ckpt")
            ...
            if key == ranker.objective_metric:
                if best_perf == None or best_perf < value:
                    ...
                    torch.save(
                        ...
                        checkpoint_path + "Bandit_best.ckpt",
                    )
                    break
    ...
    for i in range(num_iteration):
        ...
        writer.add_scalars("Score", score_summary, ranker.global_step)
        ...
        if (i + 1) % steps_per_save == 0:
            ...
            torch.save(
                ranker.model.state_dict(),
                checkpoint_path + f"Bandit(step_{ranker.global_step}).ckpt",
            )
