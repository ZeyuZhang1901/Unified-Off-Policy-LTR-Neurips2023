...
from ranker.LambdaRanker import LambdaRanker
...
def train(
    ...
    if start_checkpoint > 0:
        ...
        ckpt = torch.load(checkpoint_path + f"LambdaRank(step_{start_checkpoint}).ckpt")
            ...
            if key == ranker.objective_metric:
                if best_perf == None or best_perf < value:
                    ...
                    torch.save(
                        ...
                        checkpoint_path + "LambdaRank_best.ckpt",
                    )
                    break
    ...
    for i in range(num_iteration - start_checkpoint):
        ...
        if (i + 1) % steps_per_save == 0:
            ...
            torch.save(
                ranker.model.state_dict(),
                checkpoint_path + f"LambdaRank(step_{ranker.global_step}).ckpt",
            )
...
def test(
    ...
    ckpt = torch.load(checkpoint_path + "LambdaRank_best.ckpt")
    ranker.model.load_state_dict(ckpt)
    ranker.model.eval()
    ...
    for r in range(1, 2):
        ...
        if test_only:
            ...
            train_input_feed = Train_Input_feed(
                click_model=click_model,
                max_visuable_size=max_visuable_size,
                max_candidate_num=train_set.rank_list_size,
                batch_size=batch_size,
            )
